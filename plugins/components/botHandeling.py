...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /stream https://t.me/TBots_Father or http://t.me/TBots_Father")
    except IndexError:
        await message.reply("Usage: /stream https://t.me/TBots_Father or http://t.me/TBots_Father")
        return
...
async def m_grp(client, message):
    links = []
    link = await client.ask(message.chat.id ,"send me your pm search grp link or send /skiplink to skip , default is TBots_Father")
    if link.text == "/skiplink":
        links.append("https://t.me/TBots_Father")
    else:
        if checkIfLinkIsValid(link.text):
            links.append(link.text)
        else:
            await message.reply("Invalid link")
    link1 = await client.ask(message.chat.id ,"send me your movies grp link or send /skiplink to skip . default is TBots_Father")
